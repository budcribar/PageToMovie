...
| 11 | ☑ | Connect media folder | Settings → Project Storage (canonical) | KEEP point-of-need reconnect banners (2026-08-20, user): Settings → Project Storage remains the canonical place to connect a media folder. Film banner / Review Play tab / NavMenu reconnect / generate-gate stay as REACTIVE prompts at the moment you need the folder — not duplicates of Settings. |
...
| 14 | ☑ | New project / Import toggles | Home full-studio card | KEEP on HomeStudioCard only (2026-08-20, user): + New and Import live on the full-studio card (toolbar when a project is selected; empty-studio header too). Easy Start home is two promo cards (pick a library book / switch to full studio) and does not duplicate those buttons. Easy Start still creates a project by forking a story — a different job, not a second New/Import pair. |
| 15 | ☑ | Make movie (simple path) | SimpleVoice movie phase | KEEP BOTH (2026-08-20, user): different jobs, not duplicates. Record phase (Step 2) big "Make my movie →" (`simple-voice-make-movie`) is the NEXT step — starts the job and jumps to Step 3. Movie phase (Step 3) "Make again" (`simple-voice-remake`) is remake; "Try again" is retry on failure. |
...
| 19 | ☑ | Fork a demo ("open story") | Demo card "Fork" | KEEP BOTH (2026-08-20, user): different jobs, not duplicates. Demo gallery card explicit "Fork" (`demo-fork`) copies the screenplay into full studio. Easy Start pick list "Read in my voice →" (`simple-voice-story-{id}`) forks as a side effect so the user can record — never labeled Fork. Same copy-under-the-hood, different jobs. NOTE: forkable now requires the new Open visibility (B2) — previously-Public projects need re-marking to appear. |
