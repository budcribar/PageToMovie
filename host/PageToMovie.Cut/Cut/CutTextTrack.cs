...
public static class CutTextTrack
{
        ...
        IReadOnlyList<CutTextClip> titles)
    {
        ...
        foreach (var title in titles)
        {
            if (string.IsNullOrWhiteSpace(title.Text) && string.IsNullOrWhiteSpace(title.DisplayText))
                continue;
            var idx = OverlayLaneIndex(layout.Lanes, title.StartSec);
    ...
    /// <summary>
    /// Titles parked at the cut end belong on the last scene, not the
    /// last fraction of the previous picture.
    /// </summary>
    private static int OverlayLaneIndex(IReadOnlyList<CutTimelineLane> lanes, double startSec)
    {
        if (startSec + 0.25 >= lanes[^1].StartSec)
