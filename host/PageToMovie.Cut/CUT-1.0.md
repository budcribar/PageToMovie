...
Cut **reads** `_take_NN.mp4` + `.current.json`, in Film scene/clip order. If Film omitted the pointer, Cut may recover the highest numbered take from that exact scene/clip slot and decoder-validate it. Ignore bare `scene_SS_clip_CC.mp4` aliases. Never copy, write, or treat an alias as current.
...
Media validity and audio-only edge behavior are canonical in [media-timeline-contract.md](../../docs/media-timeline-contract.md): there is no minimum-byte validity guess; music before picture uses black frames, and music after picture freezes the final frame.
...
| Current | `scene_SS_clip_CC.current.json`; same-slot highest-take recovery only when the pointer is absent, followed by decoder validation |
