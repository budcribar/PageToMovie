...
public sealed class FilmJobService
{
    ...
    /// <summary>
    /// The predecessor take this clip was generated by continuing, or null when it was not an
    /// extend. Read from the pointer at write time, which is the take the extend input came from.
    /// </summary>
    private static int? ResolveExtendedFromTake(ClipGenContext ctx, double? providerLeadInSeconds)
    {
        if (providerLeadInSeconds is not > 0.1 || ctx.Clip <= 1)
            return null;
        var take = ClipSidecarService.ReadCurrentTake(
            Path.Combine(ctx.ProjectDir, AssetsFolder, VideoFolder), ctx.Scene, ctx.Clip - 1);
    ...
    private async Task<(string? ExtendSourcePath, string? ExtendSourceFileId, double? PredecessorDurationSec)>
    ...
    private async Task<double> ExecuteClipGenerationAsync(ClipGenContext ctx)
    {
        ...
        var lint = ShotPlanLint.Check(
            ctx.ClipEl,
            ctx.Profiles?.Values.Where(p => p.VoiceOnly).Select(p => p.Key).ToList() ?? new List<string>(),
            styleHead);
    ...
    /// stay combined — the API host never trims with native ffmpeg.</returns>
    ...
    {
        var overrunSec = 0.0;
        // Transient copy for duration probe / dialogue verify / telemetry only. Fake providers may
        ...
        finally
        {
            ...
            // telemetry and dialogue verification. The user's folder (client save via the proxy URL)
            // and the provider (source_url / file_id in the sidecar) are the durable homes. Kept only
            // under fakes, whose provider "URLs" are local fixture paths with no durable host.
        ...
        double? providerLeadInSeconds = null)
    {
        ...
        try
        {
            ...
            var sidecarPath = await _sidecars.WriteSidecarAsync(
                ...
                // Only when this clip actually continued one: it opens on that take's last frame,
                // so promoting a different take of the predecessor leaves it opening on a frame
                // that no longer exists. Recorded here so the staleness check can notice.
                extendedFromTake: ResolveExtendedFromTake(ctx, providerLeadInSeconds),
