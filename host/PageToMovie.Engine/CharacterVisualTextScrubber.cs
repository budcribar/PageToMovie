...
public static class CharacterVisualTextScrubber
{
    ...
    /// <summary>
    /// Generate-time VisualLock: drop the clothes that already live on the wardrobe list so
    /// face / markings / species remain and garments are not restated. Does not mutate seeds.
    /// The clause goes with the garment when nothing but joining words is left behind —
    /// deleting the phrase alone turned "wears a red wool coat over a grey waistcoat" into
    /// "wears a over a", and that shipped as the character's identity lock.
    /// </summary>
    public static string StripOutfitFromVisualLock(string? visualLock, IEnumerable<string>? wardrobeItems)
    {
        ...
        var garments = (wardrobeItems ?? Array.Empty<string>())
            .Where(raw => !string.IsNullOrWhiteSpace(raw))
            .SelectMany(Stage1Normalizer.CoerceStringList)
            .Where(item => item.Length >= 3)
            .ToList();
        if (garments.Count == 0)
            return CleanDebris(visualLock);

        var kept = new List<string>();
        foreach (var clause in visualLock.Split(VisualLockClauseSeparators, StringSplitOptions.None))
        {
            var text = clause;
            foreach (var item in garments)
                text = CommonRegex.Replace(text, $@"\b{Regex.Escape(item)}\b", " ", RegexOptions.IgnoreCase);
            // Only a clause that lost a garment gets its ends tidied — an untouched clause keeps
            // the article it was written with ("a livid scar over his left eye").
            if (!string.Equals(text, clause, StringComparison.Ordinal))
                text = TrimJoiningWords(text);
            text = text.Trim();
            if (text.Length > 0)
                kept.Add(text);
        }

        return CleanDebris(string.Join(", ", kept));
    }

    private static readonly char[] VisualLockClauseSeparators = [',', ';'];

    /// <summary>
    /// Whatever a removed garment left dangling at either end of its clause: "wears a", "in a",
    /// "over a". A clause that is nothing but these is gone with the garment it described.
    /// </summary>
    private static string TrimJoiningWords(string clause)
    {
        var words = clause
            ...
            .Where(w => w.Length > 0)
            .ToList();
        while (words.Count > 0 && IsJoiningWord(words[^1]))
            words.RemoveAt(words.Count - 1);
        while (words.Count > 0 && IsJoiningWord(words[0]))
            words.RemoveAt(0);
        return string.Join(" ", words).Trim();
    }

    private static bool IsJoiningWord(string word) =>
        VisualLockJoiningWords.Contains(word.Trim('.', ',', ';', ':', '-'));

    private static readonly HashSet<string> VisualLockJoiningWords =
        new(StringComparer.OrdinalIgnoreCase)
        {
            "a", "an", "the", "and", "or", "with", "in", "on", "over", "under", "of", "to",
            "wears", "wearing", "worn", "dressed", "clad", "his", "her", "their", "its",
        };
