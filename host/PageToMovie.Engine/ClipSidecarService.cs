...
public sealed class ClipSidecarService
{
    ...
    /// <summary>
    /// The clip's media on this machine: the take <c>.current.json</c> names, else the newest take
    /// file. Never the bare <c>scene_SS_clip_CC.mp4</c> alias — a leftover is not a take.
    /// <para>One resolver, because asking by file name instead of by clip is how a stored clip
    /// reads as missing.</para>
    /// </summary>
    public static string? ResolveClipMediaPath(string videoDir, int scene, int clip)
    {
        if (string.IsNullOrWhiteSpace(videoDir) || !Directory.Exists(videoDir))
            return null;

        var current = CurrentTakePath(videoDir, scene, clip);
        if (current is not null && File.Exists(current) && new FileInfo(current).Length >= MinPlayableBytes)
            return current;

        var takePrefix = ClipTakeNaming.SceneClipPrefix(scene, clip) + "_take_";
        return new DirectoryInfo(videoDir)
            .EnumerateFiles($"{takePrefix}*.mp4")
            .Where(fi => fi.Length >= MinPlayableBytes && !fi.Name.StartsWith('_'))
            .OrderByDescending(fi => fi.LastWriteTimeUtc)
            .FirstOrDefault()
            ?.FullName;
    }

    /// <summary>Smaller than this and the file is a stub, not a clip.</summary>
    public const long MinPlayableBytes = 1024;

    /// <summary>
    /// Which take of the previous clip the CURRENT take of this clip was generated by continuing,
    /// or null when it did not continue one (or predates the record).
    /// </summary>
    public static int? ReadCurrentTakeExtendedFrom(string videoDir, int scene, int clip)
    {
        var take = ReadCurrentTake(videoDir, scene, clip);
        if (take <= 0)
            return null;
        ...
        try
        {
            ...
            return doc.RootElement.TryGetProperty(ClipProviderSource.ExtendedFromTakeProperty, out var el)
                   && el.TryGetInt32(out var n) && n > 0
                ? n
                : null;
        }
        catch
        ...
        CancellationToken ct = default)
    {
        ...
        // Which take of the previous clip this one begins on — see ExtendedFromTakeProperty.
        if (extendedFromTake is > 0)
            sidecar[ClipProviderSource.ExtendedFromTakeProperty] = extendedFromTake.Value;
