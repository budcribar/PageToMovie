...
public sealed partial class ProjectStore
{
    ...
    /// <summary>The scene's heading — the key Stage 1 hashes beat ids against.</summary>
    ...
    /// </remarks>
    /// <summary>
    /// The clip that followed the deleted one no longer has the predecessor it was planned against,
    /// so its continuation is demoted to a cut and the rule stamp that vouched for it is dropped.
    /// </summary>
    /// <remarks>
    /// An <c>extend_previous</c> clip is generated by continuing the previous clip's last frame,
    /// and Stage 2 only marks one as such after checking that its action opens where the previous
    /// clip's action left the cast. Delete that predecessor and the check no longer describes
    /// anything: the successor's action assumes a moment the film no longer contains. Left alone it
    /// resolves the predecessor strictly by <c>clip - 1</c>, so it either silently generates fresh
    /// while the plan still claims a continuation, or — if a stale extend-source marker survives —
    /// continues from the deleted clip's video and opens mid-motion on a frame no viewer ever saw.
    ///
    /// <para>Dropping <c>continuity_rule</c> matters as much as the demotion: ShotPlanLint reads it
    /// to tell a checked continuation from an unchecked one, and a stamp naming a check made
    /// against a deleted clip is worse than no stamp at all.</para>
    ///
    /// <para>The stale <c>_extend_src_*</c> marker and copy are removed alongside, in
    /// <see cref="DeleteStaleExtendSource"/> — the plan saying "cut" is not enough on its own if
    /// the media layer still offers something to continue from.</para>
    /// </remarks>
    private static void DefuseSuccessorContinuation(System.Text.Json.Nodes.JsonArray clips, int index)
    {
        if (index < 0 || index >= clips.Count)
            return;
        if (clips[index] is not System.Text.Json.Nodes.JsonObject successor)
            return;
        var cont = successor[StoreLit.VeoContinuationSource]?.ToString();
        if (!string.Equals(cont, StoreLit.ExtendPrevious, StringComparison.OrdinalIgnoreCase))
            return;
        successor[StoreLit.VeoContinuationSource] = "none";
        successor.Remove(StoreLit.ContinuityRule);
    }

    private static void SyncBeatMapToClips(System.Text.Json.Nodes.JsonObject scene)
    {
        if (scene[StoreLit.Stage1BeatMap] is not System.Text.Json.Nodes.JsonArray beatMap)
            return;
        var clips = scene[StoreLit.VeoClips] as System.Text.Json.Nodes.JsonArray
                    ?? scene[StoreLit.Clips] as System.Text.Json.Nodes.JsonArray;
        if (clips is null)
        ...
        foreach (var node in clips)
        {
            var id = (node as System.Text.Json.Nodes.JsonObject)?[StoreLit.Stage1BeatId]?.ToString();
            if (string.IsNullOrWhiteSpace(id))
                return;
            ids.Add(id);
        }
    ...
    private void ApplyClipFields(
        System.Text.Json.Nodes.JsonObject clipObj,
        ClipEditRequest fields,
        string projectId,
        string? sceneSetting = null)
    {
        ...
        EnsureStage1BeatId(clipObj, fields, sceneSetting);
    ...
    /// <summary>
    /// Mints a stable beat id for a hand-added clip, keyed the same way the Stage 1 Fountain
    /// importer keys one.
    /// </summary>
    /// <remarks>
    /// This used to key on <c>"S03"</c> while the importer keys on the scene's heading, so a
    /// hand-added clip's id could never equal any id derived from the screenplay. Nothing consumed
    /// that match, so it was invisible — but it makes clip-to-paragraph mapping impossible, which
    /// is what "delete this clip's line from the screenplay" needs. <paramref name="sceneSetting"/>
    /// is optional so a caller without the scene node still gets the importer's own
    /// untitled-scene fallback rather than a third scheme.
    /// </remarks>
    ...
    private static bool RemoveClipNode(System.Text.Json.Nodes.JsonArray scenes, int scene, int clip)
    {
        foreach (var sNode in scenes)
        {
            ...
            for (var i = 0; i < clips.Count; i++)
            {
                ...
                clips.RemoveAt(i);
                DefuseSuccessorContinuation(clips, i);
    ...
    /// <summary>
    /// Removes the next clip's extend-source marker and staged copy.
    /// </summary>
    /// <remarks>
    /// These are keyed to the clip being GENERATED, not the one being continued from, so deleting a
    /// clip leaves its successor's marker behind still naming the deleted clip's provider file. The
    /// plan is demoted to a cut in <see cref="DefuseSuccessorContinuation"/>, but the marker alone
    /// is enough for the generator to find something to continue from — and it would continue from
    /// a clip that is no longer in the film, opening mid-motion on a frame nobody ever saw.
    /// </remarks>
    private static void DeleteStaleExtendSource(string videoDir, int scene, int clip)
    {
        foreach (var name in new[]
                 {
                     $"_extend_src_s{scene:D2}c{clip:D2}.json",
        ...
        CancellationToken ct)
    {
        ...
        foreach (var c in vc.EnumerateArray())
        {
            ...
            clips.Add(await BuildClipSummaryAsync(
                projectId, sceneNumber, JsonStr(sEl, StoreLit.Setting), projectDir, c, cn,
                videoIndex, mediaPresence, probeDurations, ct).ConfigureAwait(false));
        }
        return (clips, duplicateClipNumbers);
    }
        ...
        CancellationToken ct)
    {
        ...
        var (stage1BeatId, stage1BeatIds) = ResolveStage1BeatIds(
            c, sceneNumber, sceneSetting, audio.Dialogue, audio.Speaker, visualPrompt);
        ...
        JsonElement c, int sceneNumber, string? sceneSetting, string dialogue, string? speaker, string visualPrompt)
    {
            ...
            (!string.IsNullOrWhiteSpace(dialogue) || !string.IsNullOrWhiteSpace(visualPrompt)))
        {
            ...
            // Keyed on the scene heading like the Stage 1 importer, not "S03" — an id derived from
            // a different key can never match the screenplay beat it stands for.
            stage1BeatId = PageToMovie.Core.Utils.StableBeatId.ForContent(
                PageToMovie.Core.Utils.BeatIdSequencer.SceneKey(sceneSetting, sceneNumber),
                kind, speaker, string.IsNullOrWhiteSpace(dialogue) ? visualPrompt : dialogue);
        }
    ...
    /// <summary>
    /// True when this clip was generated by continuing a take of the previous clip that is no
    /// longer the current one — so it opens on a frame the film no longer contains.
    /// </summary>
    /// <remarks>
    /// Computed from present state rather than flagged when a take is promoted. A flag written at
    /// promote time would rot the other way round: promote a different take and back again, and the
    /// flag would still claim a problem that had resolved itself. This clears itself, and it catches
    /// every cause — promote, regenerate, restore from trash, revert a scene.
    ///
    /// <para>Silent for a clip that never continued anything, and for one generated before the take
    /// was recorded: an absent value means "unknown", not "wrong".</para>
    /// </remarks>
    private static bool PredecessorTakeChanged(string videoDir, int scene, int clip)
    {
        if (clip <= 1)
            return false;
        var recorded = ClipSidecarService.ReadCurrentTakeExtendedFrom(videoDir, scene, clip);
        if (recorded is not > 0)
            return false;
        var predecessorNow = ClipSidecarService.ReadCurrentTake(videoDir, scene, clip - 1);
        return predecessorNow > 0 && predecessorNow != recorded;
    }
        ...
        bool predecessorChanged = false)
    {
        ...
        if (predecessorChanged)
            return (true, "predecessor_changed");
        try
        {
