...
5. **Models catalog is the single source of truth (no provider/model assumptions in code).**
   ...
   - **Stale project config:** if a saved model id is missing or not in the catalog, **fail fast** with a clear error naming the capability and the bad id. Do not reset to `capabilities[].defaultModelId`. Required slots (video, image, chat, vision) also fail when empty. Optional slots may stay unset/`none`; an unknown non-empty id still fails.
...
8. **No hardcoded model attributes in product code — including ids, provider/key strings (`"grok"`, `"gemini"`, …), env names, URLs, or Grok\\* (or other model-branded) type dependencies.** Bind behavior at runtime from the model class / `SupportedModelCatalog` (provider / `providerId` / `requiredEnvKeys` / flags). Product call sites (media proxy, recovery, `FilmJobService`, endpoints) depend only on capability interfaces (`IVideoClient`, `IImageClient`, `IChatClient`, `IVisionClient`, `IVideoEditClient`) and the catalog-routed multi-provider facade. Concrete adapters (`GrokVideoClient`, `GeminiVideoClient`, `FalVideoClient`, …) may exist and register from catalog provider attributes; callers must not take those types, call `GetKeyAsync(..., "grok")`, or switch on a provider/model string. Recovery keys and Files downloads use the same `IVideoClient` path video generation uses for the selected model. **Video routing:** model id → `SupportedModelCatalog.Find` → catalog `providerId` → dictionary of `IVideoClient` implementations. **Fail fast — no fallbacks:** missing model id, unknown or disabled catalog row, or no `IVideoClient` in the map for that `providerId` → throw. Do not default to Grok, the first configured provider, or URL-host inference. Do not `switch` on `ModelProviderFamily`. Do not hardcode request-id prefixes (`grok:`, `gemini:`, `fal:`). A new catalog provider is a new map entry, not a new if-branch.
...
- Take files are `scene_SS_clip_CC_take_NN.mp4` + matching `.clip.json`. Take number SSoT is `ParseTakeNumber(filename)` else sidecar `take`.
- Current take SSoT is `scene_SS_clip_CC.current.json` only (which take number is current).
- Do NOT write, refresh, or play `scene_SS_clip_CC.mp4` as a current alias. That second file was going stale (Mary19). Play, stitch, compare, media-sync, AI Edit, credits, and regen must resolve `take_NN` via `.current.json`.
- Legacy alias files may still exist on disk; treat them as leftovers. Do not overwrite them as the player file. Optional: stop creating new ones; a one-time ignore is enough.

---

*Last updated: 2026-08-22 — take identity is `_take_NN`; current take is `.current.json` only; leftover `scene_SS_clip_CC.mp4` aliases are ignored.*
